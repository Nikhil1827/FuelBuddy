...
- (void)saveMasterContext {
    [self.masterManagedObjectContext performBlockAndWait:^{
        ...
        if (!saved) {
            ...
            CLSLog(@"Could not save master context due to %@", error);
        }
    }];
}
...
- (void)saveBackgroundContext {
    [self.backgroundManagedObjectContext performBlockAndWait:^{
        ...
        if (!saved) {
            // do some real error handling 
            CLSLog(@"Could not save background context due to %@", error);
        }
    }];
}
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"FuelBuddy" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator*) __persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"FuelBuddy.sqlite"];
    NSError *error = nil;
    
    // Check if we already have a compatible persistent store
    if ( [[NSFileManager defaultManager] fileExistsAtPath: [storeURL path]] ) {
        NSDictionary *existingPersistentStoreMetadata =[NSPersistentStoreCoordinator metadataForPersistentStoreOfType:NSSQLiteStoreType URL:storeURL options:options error:&error];
        if ( !existingPersistentStoreMetadata ) {
            ...
            CLSLog(@"Printing existingPersistentStoreMetadata : %@", existingPersistentStoreMetadata);
            CLSLog (@"Failed to read metadata for persistent store %@: %@", storeURL, error);
        }
        
        //Nikhil 25June2018 commented below code as it was deleting store unnecessarily!
        //Delete the file if it is not compatible
//        if ( ![self.managedObjectModel isConfiguration: nil compatibleWithStoreMetadata: existingPersistentStoreMetadata] ) {
//            if ( ![[NSFileManager defaultManager] removeItemAtURL: storeURL error: &error] )
//                CLSLog(@"*** Could not delete persistent store, %@", error);
//        } // else the existing persistent store is compatible with the current model - nice!
   ...
   _persistentStoreCoordinator = [[NSPersistentStoreCoordinator alloc] initWithManagedObjectModel:[self managedObjectModel]];
    ...
    return _persistentStoreCoordinator;//always return a full fledged coordinator
}
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    if (![_persistentStoreCoordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:storeURL options:options error:&error]) {
        ...
        // Lightweight migration will only work for a limited set of schema changes; consult "Core Data Model Versioning and Data Migration Programming Guide" for details.
         ...
         // end comment here
        NSString *failureReason = @"There was an error adding store.";
