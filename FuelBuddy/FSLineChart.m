...
- (void)commonInit
{
    ...
    self.backgroundColor = [self colorFromHexString:@"#2c2c2c"];
...
- (void)setDefaultParameters
{
    ...
    _axisColor = [UIColor lightGrayColor];
    ...
    _indexLabelFont = [UIFont fontWithName:@"HelveticaNeue-Bold" size:10];
    
    
    _valueLabelBackgroundColor = [UIColor clearColor];
    _valueLabelTextColor = [UIColor lightGrayColor];
    _valueLabelFont = [UIFont fontWithName:@"HelveticaNeue-Bold" size:10];
...
- (UILabel*)createLabelForValue: (NSUInteger)index
{
    ...
    CGPoint p = CGPointMake(_margin + (_valueLabelPosition == ValueLabelLeft ? _axisWidth : 0), _axisHeight + _margin - (index + 1) * _axisHeight / _verticalGridStep);
    
    
    NSString* text;
   // NSLog(@"min bound ++++++ %f",minBound);
    //text = _labelForValue (minBound + index);
    text = _labelForValue(minBound + (maxBound - minBound) / _verticalGridStep * (index+1));
    
     // NSString* text = _labelForValue(minBound + (maxBound - minBound)/_verticalGridStep * index);
